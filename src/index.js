...
export default{
  async fetch(req,env,ctx){
    ...
    if(url.pathname==="/"||url.pathname==="/health") return ok("OK");
    const isWS=req.headers.get("Upgrade")==="websocket";
    if((url.pathname==="/ws"||url.searchParams.get("room"))&&isWS) return this.upgradeWS(req,env);
    return notFound();
  },
  upgradeWS(req,env){
    const url=new URL(req.url);
...
export class Room{
  ...
  async fetch(req){
    ...
    try{
      ...
      if(req.headers.get("Upgrade")==="websocket"){
        ...
        this.clients.set(id,{ws:server,name,progress:0,time:0,finished:false,ready:false});
        if(!this.hostId) this.hostId=id;
        this.ensureHeartbeat();
        this.send(id,{type:"hello",id,room:this.room,players:this.playerList(),hostId:this.hostId});
        this.broadcast({type:"players",players:this.playerList(),hostId:this.hostId});

        if(this.match&&this.match.active){
          this.send(id,{type:"start",puzzles:this.match.puzzles});
        }
        ...
        server.addEventListener("message",(ev)=>{
          try{
            ...
            else if(msg.type==="ready") this.onReady(id,!!msg.v);
            else if(msg.type==="reset") this.onReset(id);
          }catch(e){}
        });
        ...
        server.addEventListener("close",async(ev)=>{
          ...
          if(this.hostId===id){
            const first=Array.from(this.clients.keys())[0]||null;
            this.hostId=first;
          }
  ...
  ensureHeartbeat(){
    ...
    this.pingTimer=setInterval(()=>{
      const s=JSON.stringify({type:"ping",t:Date.now()});
  ...
  playerList(){
    return Array.from(this.clients.entries()).map(([id,v])=>({
      id,
      name:v.name,
      progress:v.progress,
      time:v.time,
  ...
  send(id,obj){
    const v=this.clients.get(id);
    if(!v) return;
    try{ v.ws.send(JSON.stringify(obj)) }catch(e){}
  }
  broadcast(obj){
  ...
  onStart(id){
    ...
    for(const v of this.clients.values()){
      v.progress=0; v.time=0; v.finished=false; v.ready=false;
    }
    this.match={active:true,puzzles};
    this.broadcast({type:"start",puzzles});
  }
  ...
  onNext(id){
    ...
    const nextQn=v.progress+1;
    if(nextQn<1||nextQn>10) return;
    const q=this.match.puzzles[nextQn-1];
    this.send(id,{type:"question",qn:nextQn,nums:q.nums,par:q.par});
  }
  ...
  onSubmit(id,msg){
    ...
    const qn=msg.qn|0;
    if(qn!==v.progress+1){
      this.send(id,{type:"verdict",qn,ok:false,par:0,time:0,reason:"题号不匹配"});
      return;
    }
  ...
  allFinished(){
    ...
    for(const v of this.clients.values()){ if(!v.finished) return false }
    return true;
  }
  allReady(){
    if(this.clients.size===0) return false;
    for(const v of this.clients.values()){ if(!v.ready) return false }
    return true;
  }
  checkEnd(){
    if(!this.allFinished()) return;
    const ranking=this.playerList()
      .slice()
...
function comb(a,b){
  ...
  for(const op of OPS2){
    ...
    if(op.sym==="-"||op.sym==="/"){
      const v2=op.f(b,a); if(v2!==null) r.push(v2);
    }
  }
  return r;
}
    ...
    while(tries++<300){
      ...
      if(solve(nums)){
        ...
        (function dfs(list){
          ...
          for(const c of comb(a,b)){ dfs([...rest,c]); return }
        })(nums);
        return {nums,par:parFromExp(Math.max(30,exp))};
      }
    }
